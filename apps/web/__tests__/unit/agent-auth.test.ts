...
describe("agent browser authorization", () => {
	it("requires caps:read in persisted Agent API key scopes", () => {
		expect(hasAgentReadScope(["caps:read", "caps:comment"])).toBe(true);
		expect(hasAgentReadScope(["caps:comment"])).toBe(false);
		expect(hasAgentReadScope(["caps:read", "unknown:scope"])).toBe(false);
		expect(hasAgentReadScope("caps:read")).toBe(false);
	});
...
describe("agent credential usage tracking", () => {
	it("refreshes missing and stale usage timestamps", () => {
		...
		expect(agentLastUsedRefreshMs).toBe(5 * 60 * 1_000);
