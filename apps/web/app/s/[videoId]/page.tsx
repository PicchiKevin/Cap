		...
		Effect.map(
			Option.match({
				...
				onSome: ([video]) => {
					...
					const metadata = buildShareVideoMetadata(metadataInput);
					const urls = getShareVideoUrls(metadataInput);
					...
					return {
						...
						twitter: {
							...(metadata.twitter ?? {}),
							images: [urls.ogImageUrl, urls.previewImageUrl],
						},
...
async function AuthorizedContent({
	...
	const canRetryTranscriptionPermission = canRetryTranscription({
		viewerId: user?.id,
		ownerId: video.owner.id,
		canManageOrganizationSettings: canManageSharePageBranding,
	});
	...
	const videoWithOrganizationInfo = await Effect.gen(function* () {
		...
		return {
			...
			canRetryTranscription: canRetryTranscriptionPermission,
		};
	}).pipe(runPromise);
