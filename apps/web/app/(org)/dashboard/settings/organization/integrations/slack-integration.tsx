...
export function SlackIntegration({
	...
	const needsReauthorization = installations.some(
		(installation) => installation.needsReauthorization,
	);
				...
				<span
					className={
						needsReauthorization
							? "inline-flex items-center gap-1 px-2 py-0.5 text-[11px] font-medium rounded-md bg-amber-500/10 text-amber-600"
							: connected
						...
						{installations.map((installation) => (
							...
							>
								<div className="min-w-0">
									...
									{installation.needsReauthorization && (
										<p className="text-[10px] text-amber-600">
											Reconnect to enable comment notifications.
										</p>
									)}
								</div>
								<Button
