...
type IntegrationInstallationCredentials = {
	credentials: unknown;
	metadata: Record<string, unknown>;
};
...
export const listIntegrationInstallations = async ({
	...
	db()
		.select({
			...
			metadata: integrationInstallations.metadata,
...
export const getIntegrationInstallationCredentials = async ({
	provider,
	externalId,
}: {
	provider: string;
	externalId: string;
}): Promise<IntegrationInstallationCredentials | null> => {
...
export const getIntegrationCredentials = async (input: {
	provider: string;
	externalId: string;
}): Promise<unknown | null> => {
	const installation = await getIntegrationInstallationCredentials(input);
	return installation?.credentials ?? null;
};
