...
import { comments, slackCommentNotificationOutbox } from "@cap/database/schema";
import {
	provideOptionalAuth,
	Storage,
	VideosPolicy,
...
export async function newMediaComment(data: {
	...
	const notificationType = data.parentCommentId ? "reply" : "comment";
	const notificationContent =
		content || (data.kind === "video" ? "🎥 Video comment" : "🎙️ Voice note");
	...
	await db().transaction(async (tx) => {
		...
		await tx.insert(slackCommentNotificationOutbox).values({
			...
			payload: {
				videoId: data.videoId,
				authorId: user.id,
				comment: { id: data.commentId, content: notificationContent },
				parentCommentId: data.parentCommentId,
